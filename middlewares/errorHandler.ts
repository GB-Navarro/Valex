...
export default async function errorHandler(error:any, req: Request, res: Response, next: NextFunction){
    if (error.code === "error_apiKeyHasNoOwner" || error.code === "error_employeeDontExist" || error.code === "error_cardDoesNotExist" || error.code === "error_cardHasAlreadyBeenActivated"){
